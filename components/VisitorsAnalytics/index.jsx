...
export default function VisitorsAnalytics({allVisitors}) {
    ...
    let updatedOptions = {
        ...
        xaxis : {
            categories: uniqueLocation.slice(0, uniqueLocation && uniqueLocation.length > 4 ? 4 : uniqueLocation.length)
        }
    }
    const series = [
        {
            name: 'Visitor',
            data: uniqueLocation.map((locationItem)=> getAllVisitorsByCountry(allVisitors, (locationItem)))
        },
        {
            name: 'Premium Visitors',
            data: uniqueLocation.map((locationItem)=> getAllPremiumVisitorsByCountry(allVisitors, locationItem))
        },
    ]
    return (
